...
$sectionSubtitle = "Gerenciar Consultor";
$defaultPath = "/admin/consultor/";
?> 
    ...
    <section class="main__content content home flex-grid--wrap col calign-top">
        ...
        <div class="flex-grid--wrap content__box--first nopadding col-12"> @include('admin.includes.messages')
            <div class="flex-grid--wrap halign-center valign-top col-12 pd-10">
                ...
                <form class="form__maquinas form flex-grid--wrap col halign-right is-lg" action="{{ url('/admin/consultor/listar') }}" method="GET">
            ...
            <table class="table table-striped col-12 responsive-table--lg">
                ...
                <tr class="tr">
                    ...
                    <th class="th pd-10">Atendimentos</th>
                    <th class="th pd-10">Créditos</th>
                    <th class="th pd-10" style="width: 10%">Ações</th>
                </tr>
                </thead>
                <tbody class="tbody">
                @forelse ($lista as $listaItem)
                    <tr class="tr" data-id="{{ $listaItem->cd_consultor }}">
                        <td class="td pd-10 text-center" data-th="#"><p class="col"> {{ $listaItem->cd_consultor }}</p></td>
                        <td class="td pd-10" data-th="Nome"><p class="col"> {{ $listaItem->nm_consultor }}</p></td>
                        <td class="td pd-10" data-th="Status"><p class="col"> {{ $listaItem->status==1 ? "Ativado" : "Desativado" }}</p></td>
                        <td class="td pd-10" data-th="Atendimentos">
                            <p class="col">
                                <form action="{{url('/admin/atendimento/')}}" method="GET">
                                    <input type="hidden" name="consultor" id="consultor" value="{{$listaItem->cd_consultor}}">
                                    <button type="submit" class="btn--warning btn-small text-center btn-noborder btn-nomargin font-small col-0 relative">
                                        <i class="fa fa-list"></i>
                                    </button>
                                </form>
                            </p>
                        </td>
                        <td class="td pd-10" data-th="Status"><p class="col"> R$ {{ number_format($listaItem->qt_creditos, 2) }}</p></td>
                        <td class="td pd-10" data-th="Ações">
                            <div class="flex-grid col-0 valign-middle">
                                <a href="{{ url('/') }}{{ $defaultPath }}visualizar/{{ $listaItem->cd_consultor }}" class="btn--info btn-small btn-noborder btn-nomargin font-small col-0 relative">
                                    <i class="fa fa-eye"></i>
                                </a>

                                <a href="{{ url('/') }}{{ $defaultPath }}editar/{{ $listaItem->cd_consultor }}" class="btn--success btn-small btn-noborder btn-nomargin font-small col-0 mg-10--left relative">
                ...
                @empty
                    <td colspan="5" class="text-center">Não há consultores</td>
                @endforelse
                </tbody>
            </table>
            {{ $lista->links() }}
